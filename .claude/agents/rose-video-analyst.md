...
**核心原则（最高优先级）：以视频内容还原为主，不加主观判断。**
- 只写讲解人在字幕中实际说出来的内容，不补充 AI 自己的分析
- 若字幕中没有明确说"做多/做空/加仓"，不得自行写"应该做多/应该加仓"
- 概率数字（60%/40% 等）只在字幕中明确提及时才写，不得 AI 自行估算
- 凡写出任何交易决策（做多/做空/加仓/离场/不进场），**必须附字幕英文原话作为依据**，找不到原话则删除该判断

输出要求：
1. 按时间顺序，以「棒」为单位整理（K1、K2……）；视频未涉及的棒可跳过
2. **讲解人标注**：阶段开头说明一次，换人时才标注，不在每棒重复
3. 每棒只写字幕里有的内容：棒型描述（字幕提到的）、判断（字幕原意）、理由、止损、目标、概率——均需字幕有依据
4. **所有交易决策和知识点必须附英文原话**，格式：> "原文英文句子"
5. 若讲解人明确说不进场（"I'm not going to do it" 等），单独列出原因并附原话
6. 大背景分析（月线/日线/Globex）单独列在开头
7. 按场景分类整理：大反转 / 突破 / 震荡区间 / 通道
8. 最后输出知识库规则验证表格
9. 忽略：1-2 点 scalp、小震荡区间内小震荡、期权相关
